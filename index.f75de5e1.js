...
function changeSlide(number) {
    ...
    if (index === 0) images[4].classList.add('gallery__image--notActive');
    index += number;
    if (index === dotted.length) index = 0;
    if (index === -1) index = dotted.length - 1;
    dotted[index].classList.add('gallery__dot--active');
    images[index].classList.add('gallery__image--active');
    if (index === 0) images[4].classList.remove('gallery__image--notActive');
}
